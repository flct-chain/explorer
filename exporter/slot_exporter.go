...
func RunSlotExporter(client rpc.Client, firstRun bool) error {
	...
	for _, dbSlot := range dbNonFinalSlots {
		...
		logger.Infof("nodeSlotFinalized: %v dbSlot.Finalized: %w", nodeSlotFinalized, dbSlot.Finalized)

		logger.Infof("dbSlot.Slot: %v head.FinalizedSlot: %v", dbSlot.Slot, head.FinalizedSlot)
