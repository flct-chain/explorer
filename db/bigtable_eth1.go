...
func (bigtable *Bigtable) GetEth1TxsForAddressApi(prefix string, limit int64) ([]*types.Eth1TransactionIndexed, []string, error) {
...
func (bigtable *Bigtable) GetAddressTransactions(address []byte, pageToken string) (*types.TransactionDataApiResponse, error) {
	...
	transactions, keys, err := BigtableClient.GetEth1TxsForAddressApi(pageToken, DefaultInfScrollRows)
	if err != nil {
		return nil, err
	}
	...
	for i, transaction := range transactions {
		// Initialize temp for each transaction
		temp := &types.Eth1TransactionApiIndexed{
			Hash:        fmt.Sprintf("0x%x", transaction.Hash),
			BlockNumber: transaction.BlockNumber,
			Time:        uint64(transaction.Time.AsTime().Unix()),
			From:        fmt.Sprintf("0x%x", transaction.From),
			To:          fmt.Sprintf("0x%x", transaction.To),
			// Value:       utils.GetAmount(new(big.Int).SetBytes(transaction.Value), utils.Config.Frontend.ElCurrency, 6, 0, true, false, false),
			Value:    byteToDecimalString(transaction.Value),
			TxFee:    byteToDecimalString(transaction.TxFee),
			GasPrice: byteToDecimalString(transaction.GasPrice),
		}
...
func (bigtable *Bigtable) GetAddressErc20TableDataApi(address []byte, pageToken string, queryToken string) (*types.Erc20TransactionDataApiResponse, error) {
	...
	for _, transaction := range transactions {
		// Initialize temp for each transaction
		temp := &types.Erc20TransactionApiIndexed{
			Hash:        fmt.Sprintf("0x%x", transaction.ParentHash),
			BlockNumber: transaction.BlockNumber,
			Time:        uint64(transaction.Time.AsTime().Unix()),
			From:        fmt.Sprintf("0x%x", transaction.From),
			To:          fmt.Sprintf("0x%x", transaction.To),
		...
		if queryToken != "" {
			if strings.EqualFold(strings.ToUpper(temp.Symbol), strings.ToUpper(queryToken)) {
				// Append temp to results
...
func (bigtable *Bigtable) GetMetadataForAddress(address []byte, offset uint64, limit uint64) (*types.Eth1AddressMetadata, error) {
	...
	logrus.Warnf("GetMetadataForAddress=========================================%v %v", row, address)
...
func (bigtable *Bigtable) SaveBalances(balances []*types.Eth1AddressBalance, deleteKeys []string) error {
	...
	for _, balance := range balances {
		...
		logrus.Warnf("SaveBalances=========================================%v", balance.Address)
	}
