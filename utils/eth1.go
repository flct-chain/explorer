...
func formatAmount(amount *big.Int, unit string, digits int, maxPreCommaDigitsBeforeTrim int, fullAmountTooltip bool, smallUnit bool, newLineForUnit bool) template.HTML {
	...
	displayUnit := " " + unit
	var unitDigits int
	if unit == "ETH" || unit == "Ether" || unit == "xDAI" || unit == "GNO" || unit == "PNC" {
		unitDigits = 18
	} else if unit == "GWei" {
		unitDigits = 9
	} else {
...
func GetAmount(amount *big.Int, unit string, digits int, maxPreCommaDigitsBeforeTrim int, fullAmountTooltip bool, smallUnit bool, newLineForUnit bool) string {
	// define display unit & digits used per unit max
	displayUnit := " " + unit
	var unitDigits int
	if unit == "ETH" || unit == "Ether" || unit == "xDAI" || unit == "GNO" || unit == "PNC" {
	...
	trimmedAmount, fullAmount := trimAmount(amount, unitDigits, maxPreCommaDigitsBeforeTrim, digits, false)
	fmt.Sprintf(` data-toggle="tooltip" data-placement="top" title="%s"`, fullAmount)
