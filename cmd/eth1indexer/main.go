...
func main() {
	...
	for ; ; time.Sleep(time.Second * 14) {
		...
		if int(lastBlockFromBlocksTable) == 0 && int(lastBlockFromNode) > 5000 {
			lastBlockFromBlocksTable = int(lastBlockFromNode) - 5000
		}

		if int(lastBlockFromDataTable) == 0 && int(lastBlockFromNode) > 5000 {
			lastBlockFromDataTable = int(lastBlockFromNode) - 5000
		}
